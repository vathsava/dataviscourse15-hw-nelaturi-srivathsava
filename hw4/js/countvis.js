...
CountVis.prototype.initVis = function () {
   ...
   // var visScale = d3.time.scale().domain([new Date("2014/1/12"), new Date("2014/12/12")]).range([0,550]);
    brush = d3.svg.brush()
        .x(self.xScale)
        .extent([0,0.1])
        .on("brush", brushed);
...
function brushed()
{
    ...
    var b =  brush.extent();
    var from = dateFormatter.parse(b[0].getFullYear()+"-"+ b[0].getMonth()+"-"+ b[0].getDate());
    var to = dateFormatter.parse(b[1].getFullYear()+"-"+ b[1].getMonth()+"-"+ b[1].getDate());
    prioVis.wrangleData(from,to);
    prioVis.updateVis();
